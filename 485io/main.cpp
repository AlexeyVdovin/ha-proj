...
int main(int argc, char* argv[])
{
	...
	if(argc > 5)
	{
		if(cmd == CMD_485_DATA_READ) len = strtol(argv[5], null, 0);
		else if(cmd == CMD_485_DATA_WRITE)
		{
			int n = 0;
			for(i = 5; i < argc; ++i)
			{
		...
		else if(cmd == CMD_485_REG_WRITE8)
		{
			int a = strtol(argv[5], null, 0);
			data[0] = (uint8_t)(a&0xFF);
			len = 1;
		}
		else if(cmd == CMD_485_REG_WRITE16)
		{
			int a = strtol(argv[5], null, 0);
			data[0] = (uint8_t)(a&0xFF);
		...
		else if(cmd == CMD_485_REG_WRITE32)
		{
			int a = strtol(argv[5], null, 0);
			data[0] = (uint8_t)(a&0xFF);
			data[1] = (uint8_t)((a>>8)&0xFF);
	...
	if(id >= 0 && cmd >= 0 && addr >= 0)
	{
		...
		i = proto_tx(port, pkt);
		rx_timeout = proto_get_time() + 30000; // 3s response timeout
